// ========== گیم‌سنتر بله (v33: فاز ۱+۲+۳+۵) ==========
// URL Worker: https://aged-river-6500bale-game-server.metabolicbit.workers.dev/
...
// ============================================================
// CRON SLOTS - فاز ۲: مقاوم با next_run_at
// ============================================================
const CRON_TASKS = {
  inject:       { targetHour: 6,  targetMin: 30,  func: 'engineInject' },
  ensure_lock:  { targetHour: 6,  targetMin: 35,  func: 'engineEnsureLock' },
  morning:      { targetHour: 8,  targetMin: 30,  func: 'engineMorning' },
  tarfand:      { targetHour: 11, targetMin: 0,   func: 'enginePostSection', section: 'tarfand' },
  nabz:         { targetHour: 14, targetMin: 0,   func: 'enginePostSection', section: 'nabz' },
  bazi:         { targetHour: 17, targetMin: 0,   func: 'engineBazi' },
  moma:         { targetHour: 19, targetMin: 30,  func: 'enginePostSection', section: 'moma', riddle: true },
  evening:      { targetHour: 21, targetMin: 0,   func: 'engineEvening' },
  nabard_auto:  { targetHour: 21, targetMin: 30,  func: 'engineAutoNabard' },
  recess_open:  { targetHour: 23, targetMin: 15,  func: 'engineRecess', open: true },
  recess_close: { targetHour: 23, targetMin: 40,  func: 'engineRecess', open: false },
  digest:       { targetHour: 23, targetMin: 45,  func: 'engineDigest' }
};
...
// USER MANAGEMENT
// ============================================================
async function mzGetUser(KV, id) {
...
async function engineEnsureLock(env) {
  const KV = env.GAME_KV;
  const g = await KV.get('group_main');
  if (g) { await modSetLock(env, g, true); await KV.put('recess', '0'); }
}
    ...
    if (learned.indexOf(w) === -1) {
      ...
      for (const id of Object.keys(rec.u)) {
        if (/^\d+$/.test(id)) {
          ...
          try { await mzBale(env, 'sendMessage', { chat_id: id, text: '🎉 واژه «' + w + '» که ثبت کرده بودی، با اجماع بازیکن‌ها به فرهنگ‌نامهٔ نبرد واژه‌ها اضافه شد! +' + faP(10) + ' سکه' }); } catch (e) {}
        }
      }
    }
    return true;
...
async function enginePostSection(env, sectionKey, opts) {
  await cbPost(env, sectionKey, opts);
}

async function engineBazi(env) {
  const today = new Date();
  const section = (today.getDate() % 2 === 0) ? 'bazi' : 'ai';
  await cbPost(env, section);
}
...
// ============================================================
// MZ HANDLE - فاز ۳: Refactor به ماژول‌ها
// ============================================================
...
async function handleMzBet(env, KV, st, uid, name, data) {
  ...
  if (!target) return;
  if (st.bets.find(function(b) { return b.user === uid; })) { await mzBale(env, 'sendMessage', { chat_id: uid, text: 'قبلاً پیش‌بینی کردی!' }); return; }
...
async function handleMzVote(env, KV, st, uid, data, chatId) {
  ...
  if (c.yes.length >= 2 && c.yes.length > c.no.length) {
    ...
    if (/^\d+$/.test(c.owner)) {
      ...
      try { await mzBale(env, 'sendMessage', { chat_id: c.owner, text: '🏛️ دادگاه میز کلمهٔ «' + c.w + '» رو پذیرفت! +' + faP(reward) + ' سکه بهت برگشت.' }); } catch (e) {}
    }
...
async function mzHandle(update, env, ctx) {
  ...
  if (update.message) {
    ...
    if (isGroup && msg.new_chat_members && msg.new_chat_members.length) {
      ...
      await mzBale(env, 'sendMessage', { chat_id: chat.id, text: '👋 به خانوادهٔ مرکز بازی خوش اومدی!\n\n📜 قوانین رو با /rules مطالعه کن\n🏟️ میزگرد واژه‌ها هر شب ساعت ۲۱:۳۰ به‌صورت خودکار برگزار می‌شه\n✍️ جواب‌های مسابقه فقط خصوصی به بات ارسال بشه\n\n🌟 جات رو همین حالا رزرو کن!' });
      return true;
    }
      ...
      if (!priv) {
        ...
        if (violation) {
          ...
          if (w >= 5) {
            ...
            await mzBale(env, 'sendMessage', { chat_id: chat.id, text: '⛔️ ' + (msg.from.first_name || 'کاربر') + ' به‌دلیل تکرار تخلف، از گروه حذف شد.\n\n🛡️ اولویت ما حفظ آرامش و امنیت همهٔ اعضاست.' });
          } else if (w >= 3) {
            ...
            await mzBale(env, 'sendMessage', { chat_id: chat.id, text: '🔇 ' + (msg.from.first_name || 'کاربر') + ' به‌دلیل «' + violation + '» به‌مدت یک ساعت ساکت شد.\n\n🌱 امیدواریم بعد از بازگشت، با رعایت قوانین کنار هم باشیم.' });
          } else {
            await KV.put(wkey, String(w), { expirationTtl: 86400 });
            await mzBale(env, 'sendMessage', { chat_id: chat.id, text: '⚠️ ' + (msg.from.first_name || 'کاربر') + ' عزیز، «' + violation + '» در گروه مجاز نیست.\n\n📌 اخطار ' + faP(w) + ' از ' + faP(5) + '\n🔇 پس از ' + faP(3) + ' اخطار: یک ساعت سکوت\n⛔️ پس از ' + faP(5) + ' اخطار: حذف از گروه\n\n🌷 لطفاً قوانین رو رعایت کن: /rules' });
          }
          return true;
        }
      }
    ...
    if (text === '/سکوت' && msg.reply_to_message && msg.reply_to_message.from) {
      if (String((msg.from && msg.from.id) || chat.id) === (await KV.get('admin_id'))) {
        ...
        await mzBale(env, 'sendMessage', { chat_id: chat.id, text: '🔇 کاربر به‌مدت یک ساعت ساکت شد.' });
      }
      return true;
    }
    if (text === '/اخراج' && msg.reply_to_message && msg.reply_to_message.from) {
      if (String((msg.from && msg.from.id) || chat.id) === (await KV.get('admin_id'))) {
        ...
        await mzBale(env, 'sendMessage', { chat_id: chat.id, text: '⛔️ کاربر از گروه حذف شد.' });
      }
      return true;
    }
    if (text.indexOf('/افزودن_کلمه ') === 0) {
      if (String((msg.from && msg.from.id) || chat.id) === (await KV.get('admin_id'))) {
        ...
        await mzBale(env, 'sendMessage', { chat_id: chat.id, text: '✅ به فهرست سیاه اضافه شد (' + faP(arr.length) + ' مورد).' });
      }
      return true;
    }
    if (text === '/کلمات') {
    ...
    if (text === '/ایدی' || text === '/id') {
      let t = '🆔 شناسهٔ عددی تو: ' + faP(String((msg.from && msg.from.id) || chat.id));
      if (isGroup) t += '\n🆔 شناسهٔ این گروه: ' + faP(String(chat.id));
    ...
    if (isGroup && (text === '/نبرد' || text === '/nabard')) {
      ...
      if (existing && existing.phase !== 'result') {
        ...
        if (!stale) {
          const m = '⚔️ یک میزگرد دیگه همین حالا بازه!\n\nلطفاً همون‌جا بشین یا تا پایان مسابقه صبر کن.';
      ...
      await KV.put('mz_active', JSON.stringify(active));
      await KV.put('group_main', String(chat.id));
      const sent = await mzBale(env, 'sendMessage', { chat_id: chat.id, text: '📣 میزگرد واژه‌ها — ' + mzWhen() + '\n\n👑 میزبان: ' + (msg.from.first_name || '؟') + '\n⏱ ' + faP(90) + ' ثانیه | 📝 ' + faP(6) + ' ستون | ⭐ ستون طلایی با ضریب ۲\n\n✍️ جواب‌ها خصوصی | 🏁 نتیجه عمومی\n🔮 تماشاگرها می‌تونن قهرمان رو پیش‌بینی کنن!', reply_markup: { inline_keyboard: [ [{ text: '🪑 نشستن پای میز', callback_data: 'mz_join' }, { text: '⚔️ شروع نبرد (میزبان)', callback_data: 'mz_start' }], [{ text: '📨 دعوت دوستان به گروه', callback_data: 'mz_invite' }, { text: '📖 راهنما', callback_data: 'mz_guide' }] ] } });
        ...
        if (st && st.phase === 'play') {
          ...
          if (p && !p.submitted) {
            ...
            if (p.idx >= MZ_COLS.length) {
              ...
              await mzBale(env, 'sendMessage', { chat_id: uid, text: '✅ جوابت ثبت شد!\n⚡ پاداش سرعت: +' + faP(p.timeBonus) + ' سکه\n\n⏳ منتظر بقیهٔ بازیکن‌ها می‌مونیم...' });
            } else {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/submit') {
      ...
      if (request.method === 'POST') {
        try {
          ...
          // 🎯 محاسبهٔ صحیح سکه‌ها
          let coins = 0;
          const prevBest = u.best || 0;
          const newBest = score > prevBest;
          
          // ۱. سکهٔ پایه: هر ۱۰ امتیاز = ۱ سکه (حداکثر ۵۰)
          coins += Math.min(50, Math.floor(score / 10));
          
          // ۲. بونوس رکورد جدید: +۵۰ سکه
          if (newBest && score > 0) coins += 50;
          
          // ۳. بونوس سکه‌های داخل بازی: هر ۵ سکه = ۱ سکه واقعی (حداکثر ۴۰)
    ...
    if (request.method === 'POST' && url.pathname === '/webhook') {
      try {
        ...
        if (chPost) {
          try {
            ...
            if (chText === '/ایدی' || chText === '/id') {
              await bale('sendMessage', { chat_id: chPost.chat.id, text: '🆔 شناسهٔ عددی این کانال: ' + faP(String(chPost.chat.id)) });
            ...
            if (pm.forward_from_chat && (await KV.get('group_main')) && String(pm.forward_from_chat.id) === String(await KV.get('group_main'))) {
              ...
              if (mid) {
                ...
                if (arr.length >= 3) {
                  ...
                  await modCount(KV, 'del');
                  if (target) {
                    try { await mzBale(env, 'restrictChatMember', { chat_id: pm.forward_from_chat.id, user_id: Number(target), permissions: { can_send_messages: false }, until_date: Math.floor(Date.now() / 1000) + 3600 }); } catch (e) {}
                    await modCount(KV, 'mute');
                  }
                  await bale('sendMessage', { chat_id: uid, text: '🚨 پیام با ' + faP(3) + ' گزارش حذف و فرستنده ساکت شد. ممنون از همراهی!' });
                } else {
                  await bale('sendMessage', { chat_id: uid, text: '📬 گزارشت ثبت شد (' + faP(arr.length) + ' از ' + faP(3) + ').' });
                }
        ...
        if (update.callback_query) {
          ...
          else if (data === 'myid') { msg = '🆔 شناسهٔ عددی تو: ' + faP(uid); }
          ...
          if (data === 'coins') msg = '🪙 سکه تو: ' + fa(u.coins);
            ...
            else { u.claimed[key] = 1; u.coins += 100; await saveUser(uid, u); msg = '✅ عضویت تأیید شد! +' + fa(100) + ' سکه\n🪙 موجودی: ' + fa(u.coins); }
          }
    ...
    return new Response('🎮 Bale Game Server v33 is running!');
  },

  // ============================================================
  // فاز ۲: Cron مقاوم با next_run_at
  // ============================================================
  async scheduled(event, env) {
    ...
    try {
      ...
      for (const [taskName, taskConfig] of Object.entries(CRON_TASKS)) {
        const key = 'cron_next:' + taskName;
        const nextRun = parseInt(await KV.get(key) || '0');
        
        // اگه زمان اجرا گذشته یا اولین باره
        if (nextRun === 0 || Date.now() >= nextRun) {
          const targetMinutes = taskConfig.targetHour * 60 + taskConfig.targetMin;
          const targetTimeToday = new Date(now);
          targetTimeToday.setUTCHours(taskConfig.targetHour, taskConfig.targetMin, 0, 0);
          
          // اگه امروز گذشته، فردا اجرا کن
          if (currentMinutes > targetMinutes) {
            targetTimeToday.setUTCDate(targetTimeToday.getUTCDate() + 1);
          }
          
          // تبدیل به UTC
          const utcTime = new Date(targetTimeToday.getTime() - 3.5 * 3600 * 1000);
          
          // اگه الان وقتشه (توی بازه ۱۰ دقیقه‌ای)
          if (Math.abs(currentMinutes - targetMinutes) < 10) {
            try {
              if (taskConfig.func === 'engineInject') await engineInject(env);
              else if (taskConfig.func === 'engineEnsureLock') await engineEnsureLock(env);
              else if (taskConfig.func === 'engineMorning') await engineMorning(env);
              else if (taskConfig.func === 'engineEvening') await engineEvening(env);
              else if (taskConfig.func === 'engineAutoNabard') await engineAutoNabard(env);
              else if (taskConfig.func === 'engineRecess') await engineRecess(env, taskConfig.open);
              else if (taskConfig.func === 'engineDigest') await engineDigest(env);
              else if (taskConfig.func === 'engineBazi') await engineBazi(env);
              else if (taskConfig.func === 'enginePostSection') await enginePostSection(env, taskConfig.section, { riddle: taskConfig.riddle });
              
              // زمان بعدی رو ست کن (فردا)
              await KV.put(key, String(utcTime.getTime() + 24 * 3600 * 1000), { expirationTtl: 86400 * 2 });
            } catch (e) {
              console.log('engine error', taskName, e && e.message);
            }
          }
        }
      }
    } catch (e) { console.log('cron error', e && e.message); }
    
    // MZ active tables cleanup
    ...
    for (const chatId of active) {
      ...
      else if (st.phase === 'join' && Date.now() - st.createdAt > 300000) { await mzBale(env, 'sendMessage', { chat_id: chatId, text: '😴 میزگرد برگزار نشد؛ چون کسی شروع نکرد.' }); }
